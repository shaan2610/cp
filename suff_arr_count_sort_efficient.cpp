...
int suff[400005];
void count_sort(int c[],int p[],int n)//Count sort is like partial radix sort i.e. only the second part of radix sort
{
    int cnt[n]={0},ind[n];
    f(i,n)
        cnt[c[p[i]]]++;
...
void suff_arr(string s)
{
    ...
    while((1<<k)<n)
    {
        ...
        f(i,n)
            p[i]=(p[i]-(1<<k)+n)%n;
        //f(i,n)
        //    v1.pb({{c[p[i]],c[(p[i]+(1<<k))%n]},p[i]});
        count_sort(c,p,n);
        int c1[n];
        c1[p[0]]=0;
        fr(i,1,n-1)
        {
            pii x={c[p[i]],c[(p[i]+(1<<k))%n]};
            pii y={c[p[i-1]],c[(p[i-1]+(1<<k))%n]};
            if(x==y)
                c1[p[i]]=c1[p[i-1]];
            else
                c1[p[i]]=c1[p[i-1]]+1;
