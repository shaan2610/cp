...
int solve(int a[],int n,int x)
{
	...
	if(dp[n][x]!=-1)
		return dp[n][x];
	if(a[n-1]<=x)
		return dp[n][x]=solve(a,n-1,x-a[n-1])+solve(a,n-1,x);
	else
		return dp[n][x]=solve(a,n-1,x);
}
void myth()
{
    in(n);
    in(tar);//Assigning signs + or - to every element so that their sum is equal to tar.
    inp(a,n);//Counting the number of such assosiation.
    int sum=SUM(a,n);
    if((sum-tar)%2)
    	P 0;
    else
