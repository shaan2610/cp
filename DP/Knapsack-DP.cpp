...
int solve1(int w[],int v[],int n,int mx) //TOP-DOWN APPROACH
{
	f(i,1001)
    dp1[i][0]=0;
    f(i,1001)
    ...
    fr(i,1,n)
    {
    	...
    	{
    		if(w[i-1]<=j)
    			dp1[i][j]=max(v[i-1]+dp1[i-1][j-w[i-1]],dp1[i-1][j]);
    		else
    			dp1[i][j]=dp1[i-1][j];
    	}
    }
...
int solve(int w[],int v[],int n,int mx) //MEMORIZATION APPROACH
{
	if(n==0||mx==0)
		return 0;
	if(dp[n][mx]!=-1)
		return dp[n][mx];
	if(w[n-1]<=mx)
		return dp[n][mx]=max(v[n-1]+solve(w,v,n-1,mx-w[n-1]),solve(w,v,n-1,mx));
	else
		return dp[n][mx]=solve(w,v,n-1,mx);
}
void myth()
{
    in(n);
