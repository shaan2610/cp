...
vii calc(1000);
pii solve(int a[],int tl,int tr,int l,int r,int v)
{
    if(l>r)
        return {0,0};
    if(tl==l&&tr==r)
        return calc[v];
    int tm=(tl+tr)/2;
    pii a1=solve(a,tl,tm,l,min(tm,r),2*v);
    pii a2=solve(a,tm+1,tr,max(l,tm+1),r,2*v+1);
    if(a1.F==a2.F)
        return {a1.F,a1.S+a2.S};
    else
    {
        if(a1.F>a2.F)
            return a1;
...
void update(int a[],int tl,int tr,int pos,int val,int v)
{
    if(tl==tr)
        calc[v]={val,1};
    else
    {
        int tm=(tl+tr)/2;
        if(pos<=tm)
            update(a,tl,tm,pos,val,2*v);
        else
            update(a,tm+1,tr,pos,val,2*v+1);
        pii a1=calc[2*v],a2=calc[2*v+1];
        if(a1.F==a2.F)
        calc[v]={a1.F,a1.S+a2.S};
        else
        {
            if(a1.F>a2.F)
                calc[v]=a1;
...
void build(int a[],int tl,int tr,int v)
{
    ...
    else
    {
        ...
        if(calc[2*v].F==calc[2*v+1].F)
            calc[v]={calc[2*v].F,calc[2*v].S+calc[2*v+1].S};
        else
        {
            if(calc[2*v].F>calc[2*v+1].F)
                calc[v]=calc[2*v];
...
void myth()
{
    ...
    f(i,q)
    {
        ...
        if(x==1)
        {
            pii x=solve(a,0,n-1,l,r,1);
            P x.F<<" "<<x.S<<endl;
        }
        else
            update(a,0,n-1,l,r,1);
    }
}
signed main()
{
